...
## 0.0 · คำว่า SO ในโปรเจคนี้ — อ่านก่อนแตะอะไรก็ตามที่ชื่อขึ้นต้นด้วย SO

**SO = Shipment Order = ใบปิดบรรทุก** ซึ่งมีตารางของตัวเองอยู่แล้วคือ
`DOC_SHIPMENT_HDR` · **ไม่ใช่ Sales Order**

คำนี้ถูกใช้ปนกันหลายที่จนหลงได้ง่ายมาก:

| ที่ไหน | เขียนว่า | ที่จริงหมายถึง |
| --- | --- | --- |
| ไดอะแกรม ER | ตาราง `DOC_SO_HDR` / `DOC_SO_DETAIL` | ตารางที่ฐานจริงชื่อ `DOC_DO_HDR` / `DOC_DO_DETAIL` (ใบสั่งส่ง) |
| `types/manifests.ts` ฝั่ง frontend | `soNo` — *"Sales order this drop came from"* | เขียนผิด · ที่ถูกคือเลขใบปิดบรรทุกที่จุดส่งนั้นอยู่ |
| `utils/shipmentOrders.ts` ฝั่ง frontend | `ShipmentOrder` | ชื่อ **มุมมองบนจอ** (คิวใบสั่งส่งที่รอขึ้นรถ) ไม่ใช่ตาราง |
| `DOC_DO_HDR.EXTERNORDERKEY` | — | เลขเอกสารต้นทางจาก SAP ที่ OMS ส่งต่อมา · เป็นของระบบอื่น ไม่มีตารางในฐานนี้ |

> **เคยเข้าใจผิดแล้วครั้งหนึ่ง** มีการเสนอสร้าง `DOC_SO_HDR` / `DOC_SO_DETAIL`
> เป็นชั้น "ใบสั่งขาย" ขึ้นมาใหม่ — **ถอนออกแล้วทั้งหมด** เพราะอ่านคำว่า SO ผิด
> ถ้าเจอชื่อนี้ในเอกสารเก่าหรือ commit เก่า อย่าหยิบไปใช้ต่อ
>
> วิธีตรวจว่าเข้าใจถูก: ใบปิดบรรทุกหนึ่งใบถือจุดส่งหลายจุด และแต่ละจุดมี **DO
> คนละเลข** แต่ทุกจุด **อยู่บน SO เลขเดียวกัน** คือเลขใบปิดบรรทุกนั้น
...
ไม่ออกว่าพังเพราะโครงสร้างหรือเพราะข้อมูล · `-Seed` ใส่ **366 แถวใน 20 ตาราง**
