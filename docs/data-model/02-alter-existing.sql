...
/* --- ใบสั่งส่งชี้กลับไปเอกสารต้นทาง --------------------------------------
   `DOC_DO_HDR.EXTERNORDERKEY` เก็บเลขเอกสารจากระบบต้นทาง (SAP ผ่าน OMS) เป็น
   ข้อความ และ **ยังผูก FK ไม่ได้** เพราะระบบต้นทางอยู่นอกฐานนี้ ไม่มีตารางให้ชี้

   เคยมีการเสนอสร้างตาราง `DOC_SO_HDR` มารับตรงนี้ — **ถอนออกแล้ว** เพราะ
   ในโปรเจคนี้ SO ย่อมาจาก *Shipment Order* คือใบปิดบรรทุก ซึ่งมีตารางอยู่แล้ว
   คือ `DOC_SHIPMENT_HDR` ไม่ใช่ Sales Order ตามที่เข้าใจผิดตอนแรก
   (ไดอะแกรม ER ก็ใช้ชื่อ `DOC_SO_HDR` เรียกตารางที่ฐานจริงชื่อ `DOC_DO_HDR`
   อีกที — คำว่า SO ในเอกสารชุดนี้จึงต้องอ่านตามบริบททุกครั้ง)

   ถ้าวันหนึ่งต้องตรวจว่าเลขต้นทางซ้ำหรือชี้ไปไหน ให้ทำที่ backend ตอนรับเข้า
   ไม่ใช่ที่ FK — ฐานนี้ไม่มีทางรู้ว่าเลขของ SAP ถูกหรือผิด */

/* หาใบสั่งส่งทุกใบที่มาจากเอกสารต้นทางเดียวกัน */
